...
namespace RTSNavigationLib
{
    ...
    std::vector<size_t> Usher::assignPlaces(const std::vector<WorldBody>& units, const std::vector<WorldBody>& places)
    {
        return assignPlaces2KD(units, places);
    }
    std::vector<size_t> Usher::assignPlaces2KD(const std::vector<WorldBody>& units, const std::vector<WorldBody>& places)
    {
        ...
        for (const auto& x : units)
            unitsV.push_back(x.position);
        for (const auto& x : places)
            placesV.push_back(x.position);
        return AssignmentProblemDualKDTree::getTickets(unitsV, placesV);
    }

    std::vector<size_t> Usher::assignPlacesMCF(const std::vector<WorldBody>& units, const std::vector<WorldBody>& places)
    {
    ...
    void Usher::visualize(const std::vector<size_t>& usherResult, const std::vector<WorldBody>& units, const std::vector<WorldBody>& places)
    {
        ...
        for (size_t i = 0; i < usherResult.size(); i++)
        {
            ...
            debug.streak    = { units[i].position, places[ticket].position };
            debug.color     = "yellow";
            debug.thickness = 0.1;
            svgDebug.push_back(debug);
        }
